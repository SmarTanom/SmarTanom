    ...
    def handle(self, *args, **options):
        ...
        for serial in serials:
            ...
            with transaction.atomic():
                ...
                for s in sensors:
                    # Ensure exactly one reading per sensor: skip if any reading already exists
                    if SensorData.objects.filter(sensor=s).exists():
                        continue
        ...
        self.stdout.write(self.style.SUCCESS(
            f"Seeded sensors: {created_sensors}, readings: {created_readings} for {len(serials) - len(missing_devices)} device(s)."
        ))

    @staticmethod
    def _value_for_sensor_type(sensor_type: str, *, ph_value: float, tds_value: float, wl_value: float, air_temp_value: float) -> float:
        ...
        if sensor_type == Sensor.SensorType.TDS:
            return float(tds_value)
        if sensor_type == Sensor.SensorType.EC:
            return 1.6  # mS/cm typical hydroponic target; UI shows EC Levels
        if sensor_type == Sensor.SensorType.WATER_LEVEL:
            return float(wl_value)
        if sensor_type == Sensor.SensorType.AIR_TEMPERATURE:
            return float(air_temp_value)
